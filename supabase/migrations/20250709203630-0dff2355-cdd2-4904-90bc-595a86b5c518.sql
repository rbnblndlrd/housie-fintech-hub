...
CREATE POLICY "System can manage credits" ON public.ai_credits
  FOR ALL USING (true);

-- RLS policies for ai_credit_logs  
CREATE POLICY "Users can view their own credit logs" ON public.ai_credit_logs
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "System can insert credit logs" ON public.ai_credit_logs
