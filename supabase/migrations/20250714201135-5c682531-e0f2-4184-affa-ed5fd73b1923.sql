...
CREATE TABLE public.user_equipped_stamps (
  ...
  display_position INTEGER NOT NULL CHECK (display_position >= 1 AND display_position <= 3),
  ...
  UNIQUE(user_id, display_position),
...
AS $$
  SELECT ues.stamp_id, ues.display_position, ues.equipped_at
  FROM public.user_equipped_stamps ues
  WHERE ues.user_id = p_user_id
  ORDER BY ues.display_position;
$$;