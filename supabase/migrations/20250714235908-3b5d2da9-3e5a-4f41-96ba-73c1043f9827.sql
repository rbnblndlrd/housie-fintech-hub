...
-- Insert sample fusion stamp definitions
...
('master-craftsman', 'Master Craftsman', '["quality-assured", "repeat-customer", "five-star-sweep"]', 'Excellence refined through repetition. When skill meets consistency, legends are born.', '["Fusion complete. Excellence just got evolutionary.", "Three stamps, one truth: mastery demands dedication."]', 1.5, 2),
('speed-demon', 'Speed Demon', '["blitzmaster", "clockwork", "efficiency-expert"]', 'Time bends to those who master its rhythm. Speed without sacrifice, precision without pause.', '["Velocity fused with precision. That''s how you bend time.", "Three speeds, one blur. Efficiency just got personal."]', 1.3, 1),
('canon-guardian', 'Canon Guardian', '["road-warrior", "solo-operator", "network-builder"]', 'Protector of the grid. Guardian of the Canon. When trust needs a champion, they answer.', '["Canon Guardian unlocked. The grid has its sentinel.", "Trust weaponized. Guardian mode: activated."]', 2.0, 3),
('echo-amplifier', 'Echo Amplifier', '["broadcaster", "influencer", "community-voice"]', 'Voice that carries across cities. Echo that shapes the Canon. When words become power.', '["Echo Amplifier online. Your voice just got citywide range.", "Three voices, one signal. Broadcasting excellence."]', 1.8, 2);
...
BEGIN
  ...
  -- Get user's prestige tier (simplified - you may need to adjust based on your prestige system)
  SELECT COALESCE(
    (SELECT tier FROM user_prestige WHERE user_id = p_user_id ORDER BY created_at DESC LIMIT 1),
    1
  ) INTO user_tier;
  ...
  RETURN required_stamps <@ COALESCE(user_stamps, '{}');
END;
$$;
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'voice_line', fusion_record.annette_voice_lines[1]
  );
END;
$$;
