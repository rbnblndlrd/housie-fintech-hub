...
DECLARE 
    ...
    existing_system_user_id UUID;
BEGIN
    -- Check if system user already exists with this email
    SELECT id INTO existing_system_user_id 
    FROM users 
    WHERE email = 'system@housie.ca' 
    LIMIT 1;
    
    -- If exists, use that ID, otherwise create new one
    IF existing_system_user_id IS NOT NULL THEN
        system_user_id := existing_system_user_id;
    ELSE
        -- Insert system user
