-- Create meta-chain tracking system for "Track of Tracks"
...
CREATE TABLE public.meta_chain_definitions (
  ...
  required_chain_types TEXT[] DEFAULT NULL,
...
ON public.chain_seal_timeline FOR SELECT
USING (user_id IN (
  SELECT id FROM users WHERE allow_public_timeline = true
));
...
-- Add public timeline flag to users (for Hall of Legends)
ALTER TABLE public.users ADD COLUMN IF NOT EXISTS allow_public_timeline BOOLEAN NOT NULL DEFAULT false;
